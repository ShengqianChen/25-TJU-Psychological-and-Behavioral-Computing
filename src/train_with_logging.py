...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    ...
    if hyp_params.aligned or hyp_params.model=='MULT' or 'Improved' in hyp_params.model or 'Baseline' in hyp_params.model:
        ctc_criterion = None
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        if val_metrics:
            print(f'Valid Metrics: {val_metrics}')
        if test_metrics:
    ...
    # 打印最佳epoch信息
    best_epoch, best_val_loss = logger.get_best_epoch('val_loss', 'min')
    print(f"\nBest validation loss: {best_val_loss:.4f} at epoch {best_epoch}")
    
    print(f"\nTraining log saved to: {logger.log_path}")
